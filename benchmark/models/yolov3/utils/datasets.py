...
class VocDataset(Dataset):
    def __init__(self, anno_file_type, img_size=416):
        ...
        self._annotations = self._load_annotations(anno_file_type)
    ...
    def __getitem__(self, item):
        ...
        img_org, bboxes_org = self._parse_annotation(self._annotations[item])
        img_org = img_org.transpose(2, 0, 1)  # HWC->CHW
        
        item_mix = random.randint(0, len(self._annotations)-1)
        img_mix, bboxes_mix = self._parse_annotation(self._annotations[item_mix])
        ...
        label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes = self._creat_label(bboxes)
    ...
    def _load_annotations(self, anno_type):
